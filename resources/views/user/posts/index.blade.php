...
@section('user.content')
    ...
    @if ($posts->isEmpty())
        <div class="text-center py-5">
            ...
            <p class="text-muted mt-3">{{ __('У вас пока нет постов.') }}</p>
            <x-button-link href="{{ route('user.posts.create') }}">
                {{ __('Написать первый пост') }}
            </x-button-link>
        </div>
    @else
        <div class="row g-4">
            @foreach($posts as $post)
                <div class="col-12 col-md-6">
                    <div class="card h-100 shadow-sm hover-shadow position-relative">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">
                                <a href="{{ route('user.posts.show', $post) }}" class="text-decoration-none text-dark stretched-link">
                                    {{ $post->title }}
                                </a>
                            </h5>
